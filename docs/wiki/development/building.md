...
- [HEMTT](https://github.com/BrettMayson/HEMTT) (>=v1.6.0) in project root _(Windows users can double-click `tools/setup.bat`)_
...
You will also need `acre.dll` and `ACRE2Arma.dll` (or `acre_x64.dll` and `ACRE2Arma_x64.dll` for 64-bit) placed in `idi\acre` folder. You can obtain those from last release or [build them yourself](building-extensions). Mumble/TeamSpeak plugin is not required for basic operation, however the `plugins` folder may also be placed in `idi\acre` folder.
