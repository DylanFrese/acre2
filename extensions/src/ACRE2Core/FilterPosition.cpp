...
//acre::Result CFilterPosition::process(short* samples, int sampleCount, int channels, CPlayer *player, const unsigned int* channelSpeakerArray, unsigned int *channelMask, acre::volume_t volume) {
acre::Result CFilterPosition::process(short* samples, int sampleCount, int channels, const unsigned int speakerMask, CSoundMixdownEffect *params) {
    ...
    */
    ...
    if (CEngine::getInstance()->getSoundEngine()->getCurveModel() == acre::CurveModel::amplitude || CEngine::getInstance()->getSoundEngine()->getCurveModel() == acre::CurveModel::selectableA) {
